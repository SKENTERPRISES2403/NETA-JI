...
namespace NetaJi.Prototype
{
    ...
    public sealed class PlayerProgress
    {
        ...
        public int nationalCampaignSupport;
        public int nationalCampaignCompliance;
        public int nationalElectionOperations;
        public int firstNationalVoteShare;
        public int firstNationalSeatsWon;
        public bool firstNationalElectionContested;
        public bool firstNationalElectionWon;
        public int oppositionServiceRecord;
        public int nationalAllianceRenewal;
        public int nationalPolicyCorrection;
        public int nationalComebackScore;
        public bool nationalComebackReady;
        public int secondNationalCampaignSupport;
        public int secondNationalCampaignCompliance;
        public int secondNationalElectionOperations;
        public int secondNationalVoteShare;
        public int secondNationalSeatsWon;
        public bool secondNationalElectionContested;
        public bool primeMinisterElected;
        public int primeMinisterDelivery;
        public int unionCabinetIntegrity;
        public int nationalFiscalDiscipline;
        public int institutionalTrust;
        public int primeMinisterHundredDayScore;
        public bool primeMinisterHundredDayReviewPassed;
        public int nationalHealthIndex;
        public int nationalLearningIndex;
        public int nationalSafetyJusticeIndex;
        public int nationalLivelihoodIndex;
        public int nationalDevelopmentScore;
        public bool nationalDevelopmentReviewPassed;
        public int globalTradeTrust;
        public int scienceInnovationLeadership;
        public int peaceDefenseReadiness;
        public int humanitarianClimateLeadership;
        public int globalLeadershipScore;
        public bool vishwaGuruOutcomeEarned;
        ...
        public int chapterNineteenStep;
        public int chapterTwentyStep;
        public int chapterTwentyOneStep;
        public int chapterTwentyTwoStep;
        public int chapterTwentyThreeStep;
        public int chapterTwentyFourStep;
        ...
        public bool chapterNineteenComplete;
        public bool chapterTwentyComplete;
        public bool chapterTwentyOneComplete;
        public bool chapterTwentyTwoComplete;
        public bool chapterTwentyThreeComplete;
        public bool chapterTwentyFourComplete;
        ...
        public int firstNationalCampaignApproach;
        public int oppositionTermApproach;
        public int secondNationalCampaignApproach;
        public int primeMinisterGovernanceApproach;
        public int nationalDevelopmentApproach;
        public int globalLeadershipApproach;
    ...
    public sealed class GameSession : MonoBehaviour
    {
        ...
        public void ApplyFirstNationalCampaignReward(int support, int compliance, int operations)
        {
            progress.nationalCampaignSupport = Mathf.Clamp(progress.nationalCampaignSupport + support, 0, 100);
            progress.nationalCampaignCompliance = Mathf.Clamp(progress.nationalCampaignCompliance + compliance, 0, 100);
            progress.nationalElectionOperations = Mathf.Clamp(progress.nationalElectionOperations + operations, 0, 100);
            Save();
        }

        public bool ResolveFirstNationalElection()
        {
            float voteShare = 12f
                + progress.nationalCampaignSupport * 0.16f
                + progress.nationalCampaignCompliance * 0.08f
                + progress.nationalElectionOperations * 0.08f
                + progress.nationalReadinessScore * 0.07f
                + progress.federalAllianceTrust * 0.04f
                + progress.reputation * 0.02f
                + progress.caseProof * 0.01f
                - progress.oppositionPressure * 0.06f;
            progress.firstNationalVoteShare = Mathf.Clamp(Mathf.RoundToInt(voteShare), 25, 65);
            progress.firstNationalSeatsWon = Mathf.Clamp(
                Mathf.RoundToInt((progress.firstNationalVoteShare - 20f) * 1.45f), 0, 100);
            progress.firstNationalElectionContested = progress.nationalExpansionReady
                && progress.nationalCampaignCompliance >= 60
                && progress.nationalElectionOperations >= 60;
            progress.firstNationalElectionWon = progress.firstNationalElectionContested
                && progress.firstNationalVoteShare >= 50
                && progress.firstNationalSeatsWon >= 51;
            Save();
            return progress.firstNationalElectionWon;
        }

        public void ApplyOppositionTermReward(int service, int allianceRenewal, int policyCorrection)
        {
            progress.oppositionServiceRecord = Mathf.Clamp(progress.oppositionServiceRecord + service, 0, 100);
            progress.nationalAllianceRenewal = Mathf.Clamp(progress.nationalAllianceRenewal + allianceRenewal, 0, 100);
            progress.nationalPolicyCorrection = Mathf.Clamp(progress.nationalPolicyCorrection + policyCorrection, 0, 100);
            Save();
        }

        public bool ResolveNationalComeback()
        {
            float score = progress.oppositionServiceRecord * 0.30f
                + progress.nationalAllianceRenewal * 0.27f
                + progress.nationalPolicyCorrection * 0.23f
                + progress.nationalReadinessScore * 0.08f
                + progress.firstNationalVoteShare * 0.05f
                + progress.reputation * 0.03f
                + progress.caseProof * 0.02f
                - progress.oppositionPressure * 0.05f;
            progress.nationalComebackScore = Mathf.Clamp(Mathf.RoundToInt(score), 0, 100);
            progress.nationalComebackReady = progress.firstNationalElectionContested
                && !progress.firstNationalElectionWon
                && progress.nationalAllianceRenewal >= 60
                && progress.nationalPolicyCorrection >= 60
                && progress.nationalComebackScore >= 74;
            Save();
            return progress.nationalComebackReady;
        }

        public void ApplySecondNationalCampaignReward(int support, int compliance, int operations)
        {
            progress.secondNationalCampaignSupport = Mathf.Clamp(progress.secondNationalCampaignSupport + support, 0, 100);
            progress.secondNationalCampaignCompliance = Mathf.Clamp(progress.secondNationalCampaignCompliance + compliance, 0, 100);
            progress.secondNationalElectionOperations = Mathf.Clamp(progress.secondNationalElectionOperations + operations, 0, 100);
            Save();
        }

        public bool ResolveSecondNationalElection()
        {
            float voteShare = 16f
                + progress.secondNationalCampaignSupport * 0.14f
                + progress.secondNationalCampaignCompliance * 0.08f
                + progress.secondNationalElectionOperations * 0.08f
                + progress.nationalComebackScore * 0.08f
                + progress.nationalAllianceRenewal * 0.05f
                + progress.nationalPolicyCorrection * 0.04f
                + progress.reputation * 0.02f
                + progress.caseProof * 0.01f
                + progress.firstNationalVoteShare * 0.03f
                - progress.oppositionPressure * 0.06f;
            progress.secondNationalVoteShare = Mathf.Clamp(Mathf.RoundToInt(voteShare), 25, 65);
            progress.secondNationalSeatsWon = Mathf.Clamp(
                Mathf.RoundToInt((progress.secondNationalVoteShare - 20f) * 1.60f), 0, 100);
            progress.secondNationalElectionContested = progress.nationalComebackReady
                && progress.secondNationalCampaignCompliance >= 60
                && progress.secondNationalElectionOperations >= 60;
            progress.primeMinisterElected = progress.secondNationalElectionContested
                && progress.secondNationalVoteShare >= 50
                && progress.secondNationalSeatsWon >= 51;
            Save();
            return progress.primeMinisterElected;
        }

        public void ApplyPrimeMinisterGovernanceReward(int delivery, int cabinetIntegrity, int fiscalDiscipline, int institutions)
        {
            progress.primeMinisterDelivery = Mathf.Clamp(progress.primeMinisterDelivery + delivery, 0, 100);
            progress.unionCabinetIntegrity = Mathf.Clamp(progress.unionCabinetIntegrity + cabinetIntegrity, 0, 100);
            progress.nationalFiscalDiscipline = Mathf.Clamp(progress.nationalFiscalDiscipline + fiscalDiscipline, 0, 100);
            progress.institutionalTrust = Mathf.Clamp(progress.institutionalTrust + institutions, 0, 100);
            Save();
        }

        public bool ResolvePrimeMinisterHundredDayReview()
        {
            float score = progress.primeMinisterDelivery * 0.30f
                + progress.unionCabinetIntegrity * 0.23f
                + progress.nationalFiscalDiscipline * 0.20f
                + progress.institutionalTrust * 0.17f
                + progress.reputation * 0.04f
                + progress.caseProof * 0.03f
                + progress.secondNationalVoteShare * 0.05f
                - progress.oppositionPressure * 0.05f;
            progress.primeMinisterHundredDayScore = Mathf.Clamp(Mathf.RoundToInt(score), 0, 100);
            progress.primeMinisterHundredDayReviewPassed = progress.primeMinisterElected
                && progress.primeMinisterDelivery >= 65
                && progress.unionCabinetIntegrity >= 65
                && progress.nationalFiscalDiscipline >= 65
                && progress.institutionalTrust >= 65
                && progress.primeMinisterHundredDayScore >= 75;
            Save();
            return progress.primeMinisterHundredDayReviewPassed;
        }

        public void ApplyNationalDevelopmentReward(int health, int learning, int safetyJustice, int livelihood)
        {
            progress.nationalHealthIndex = Mathf.Clamp(progress.nationalHealthIndex + health, 0, 100);
            progress.nationalLearningIndex = Mathf.Clamp(progress.nationalLearningIndex + learning, 0, 100);
            progress.nationalSafetyJusticeIndex = Mathf.Clamp(progress.nationalSafetyJusticeIndex + safetyJustice, 0, 100);
            progress.nationalLivelihoodIndex = Mathf.Clamp(progress.nationalLivelihoodIndex + livelihood, 0, 100);
            Save();
        }

        public bool ResolveNationalDevelopmentReview()
        {
            float score = progress.nationalHealthIndex * 0.23f
                + progress.nationalLearningIndex * 0.22f
                + progress.nationalSafetyJusticeIndex * 0.20f
                + progress.nationalLivelihoodIndex * 0.19f
                + progress.primeMinisterHundredDayScore * 0.08f
                + progress.institutionalTrust * 0.04f
                + progress.caseProof * 0.02f
                - progress.oppositionPressure * 0.04f;
            progress.nationalDevelopmentScore = Mathf.Clamp(Mathf.RoundToInt(score), 0, 100);
            progress.nationalDevelopmentReviewPassed = progress.primeMinisterHundredDayReviewPassed
                && progress.nationalHealthIndex >= 68
                && progress.nationalLearningIndex >= 68
                && progress.nationalSafetyJusticeIndex >= 68
                && progress.nationalLivelihoodIndex >= 68
                && progress.nationalDevelopmentScore >= 82;
            Save();
            return progress.nationalDevelopmentReviewPassed;
        }

        public void ApplyGlobalLeadershipReward(int trade, int science, int peaceDefense, int humanitarianClimate)
        {
            progress.globalTradeTrust = Mathf.Clamp(progress.globalTradeTrust + trade, 0, 100);
            progress.scienceInnovationLeadership = Mathf.Clamp(progress.scienceInnovationLeadership + science, 0, 100);
            progress.peaceDefenseReadiness = Mathf.Clamp(progress.peaceDefenseReadiness + peaceDefense, 0, 100);
            progress.humanitarianClimateLeadership = Mathf.Clamp(progress.humanitarianClimateLeadership + humanitarianClimate, 0, 100);
            Save();
        }

        public bool ResolveGlobalLeadershipReview()
        {
            float score = progress.globalTradeTrust * 0.22f
                + progress.scienceInnovationLeadership * 0.23f
                + progress.peaceDefenseReadiness * 0.20f
                + progress.humanitarianClimateLeadership * 0.23f
                + progress.nationalDevelopmentScore * 0.08f
                + progress.institutionalTrust * 0.04f
                + progress.reputation * 0.03f
                - progress.oppositionPressure * 0.04f;
            progress.globalLeadershipScore = Mathf.Clamp(Mathf.RoundToInt(score), 0, 100);
            progress.vishwaGuruOutcomeEarned = progress.nationalDevelopmentReviewPassed
                && progress.globalTradeTrust >= 75
                && progress.scienceInnovationLeadership >= 75
                && progress.peaceDefenseReadiness >= 75
                && progress.humanitarianClimateLeadership >= 75
                && progress.globalLeadershipScore >= 88;
            Save();
            return progress.vishwaGuruOutcomeEarned;
        }
        ...
        public int GetMissionStep(int chapterNumber)
        {
            ...
            {
                case 24:
                    return progress.chapterTwentyFourStep;
                case 23:
                    return progress.chapterTwentyThreeStep;
                case 22:
                    return progress.chapterTwentyTwoStep;
                case 21:
                    return progress.chapterTwentyOneStep;
                case 2:
                ...
                case 19:
                    return progress.chapterNineteenStep;
                case 20:
                    return progress.chapterTwentyStep;
                default:
        ...
        public void SetMissionStep(int chapterNumber, int step)
        {
            ...
            if (chapterNumber == 24)
            {
                progress.chapterTwentyFourStep = safeStep;
            }
            else if (chapterNumber == 23)
            {
                progress.chapterTwentyThreeStep = safeStep;
            }
            else if (chapterNumber == 22)
            {
                progress.chapterTwentyTwoStep = safeStep;
            }
            else if (chapterNumber == 21)
            {
                progress.chapterTwentyOneStep = safeStep;
            }
            else if (chapterNumber == 20)
            {
                progress.chapterTwentyStep = safeStep;
            }
            else if (chapterNumber == 19)
            {
                progress.chapterNineteenStep = safeStep;
            }
            ...
            progress.lastPlayedChapter = Mathf.Clamp(chapterNumber, 1, 24);
            Save();
        }
        ...
        public void ResetChapter(int chapterNumber)
        {
            if (chapterNumber == 24)
            {
                progress.globalLeadershipApproach = 0;
                progress.globalTradeTrust = 0;
                progress.scienceInnovationLeadership = 0;
                progress.peaceDefenseReadiness = 0;
                progress.humanitarianClimateLeadership = 0;
                progress.globalLeadershipScore = 0;
                progress.vishwaGuruOutcomeEarned = false;
            }
            else if (chapterNumber == 23)
            {
                progress.nationalDevelopmentApproach = 0;
                progress.nationalHealthIndex = 0;
                progress.nationalLearningIndex = 0;
                progress.nationalSafetyJusticeIndex = 0;
                progress.nationalLivelihoodIndex = 0;
                progress.nationalDevelopmentScore = 0;
                progress.nationalDevelopmentReviewPassed = false;
            }
            else if (chapterNumber == 22)
            {
                progress.primeMinisterGovernanceApproach = 0;
                progress.primeMinisterDelivery = 0;
                progress.unionCabinetIntegrity = 0;
                progress.nationalFiscalDiscipline = 0;
                progress.institutionalTrust = 0;
                progress.primeMinisterHundredDayScore = 0;
                progress.primeMinisterHundredDayReviewPassed = false;
            }
            else if (chapterNumber == 21)
            {
                progress.secondNationalCampaignApproach = 0;
                progress.secondNationalCampaignSupport = 0;
                progress.secondNationalCampaignCompliance = 0;
                progress.secondNationalElectionOperations = 0;
                progress.secondNationalVoteShare = 0;
                progress.secondNationalSeatsWon = 0;
                progress.secondNationalElectionContested = false;
                progress.primeMinisterElected = false;
            }
            else if (chapterNumber == 20)
            {
                progress.oppositionTermApproach = 0;
                progress.oppositionServiceRecord = 0;
                progress.nationalAllianceRenewal = 0;
                progress.nationalPolicyCorrection = 0;
                progress.nationalComebackScore = 0;
                progress.nationalComebackReady = false;
            }
            else if (chapterNumber == 19)
            {
                progress.firstNationalCampaignApproach = 0;
                progress.nationalCampaignSupport = 0;
                progress.nationalCampaignCompliance = 0;
                progress.nationalElectionOperations = 0;
                progress.firstNationalVoteShare = 0;
                progress.firstNationalSeatsWon = 0;
                progress.firstNationalElectionContested = false;
                progress.firstNationalElectionWon = false;
            }
            else if (chapterNumber == 18)
            {
        ...
        public void CompleteChapter(int chapterNumber)
        {
            if (chapterNumber == 24)
            {
                progress.chapterTwentyFourComplete = true;
                progress.lastPlayedChapter = 24;
            }
            else if (chapterNumber == 23)
            {
                progress.chapterTwentyThreeComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 24);
                progress.lastPlayedChapter = 24;
            }
            else if (chapterNumber == 22)
            {
                progress.chapterTwentyTwoComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 23);
                progress.lastPlayedChapter = 23;
            }
            else if (chapterNumber == 21)
            {
                progress.chapterTwentyOneComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 22);
                progress.lastPlayedChapter = 22;
            }
            else if (chapterNumber == 20)
            {
                progress.chapterTwentyComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 21);
                progress.lastPlayedChapter = 21;
            }
            else if (chapterNumber == 19)
            {
                progress.chapterNineteenComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 20);
                progress.lastPlayedChapter = 20;
            }
            else if (chapterNumber == 18)
            {
                progress.chapterEighteenComplete = true;
                progress.highestUnlockedChapter = Mathf.Max(progress.highestUnlockedChapter, 19);
                progress.lastPlayedChapter = 19;
            }
            else if (chapterNumber == 17)
            {
        ...
        public void SetStoryDecision(string key, int option)
        {
            ...
            else if (key == "first-national-campaign-approach")
            {
                progress.firstNationalCampaignApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
            else if (key == "opposition-term-approach")
            {
                progress.oppositionTermApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
            else if (key == "second-national-campaign-approach")
            {
                progress.secondNationalCampaignApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
            else if (key == "pm-governance-approach")
            {
                progress.primeMinisterGovernanceApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
            else if (key == "national-development-approach")
            {
                progress.nationalDevelopmentApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
            else if (key == "global-leadership-approach")
            {
                progress.globalLeadershipApproach = Mathf.Clamp(option, 1, 2);
                Save();
            }
        }
        ...
        private void MigrateProgress()
        {
            ...
            if (progress.chapterEighteenComplete && progress.highestUnlockedChapter < 19)
            {
                progress.highestUnlockedChapter = 19;
                changed = true;
            }
            if (progress.chapterNineteenStep >= 12 && !progress.chapterNineteenComplete)
            {
                progress.chapterNineteenComplete = true;
                changed = true;
            }
            if (progress.chapterNineteenComplete && progress.highestUnlockedChapter < 20)
            {
                progress.highestUnlockedChapter = 20;
                changed = true;
            }
            if (progress.chapterTwentyStep >= 12 && !progress.chapterTwentyComplete)
            {
                progress.chapterTwentyComplete = true;
                changed = true;
            }
            if (progress.chapterTwentyComplete && progress.highestUnlockedChapter < 21)
            {
                progress.highestUnlockedChapter = 21;
                changed = true;
            }
            if (progress.chapterTwentyOneStep >= 12 && !progress.chapterTwentyOneComplete)
            {
                progress.chapterTwentyOneComplete = true;
                changed = true;
            }
            if (progress.chapterTwentyOneComplete && progress.highestUnlockedChapter < 22)
            {
                progress.highestUnlockedChapter = 22;
                changed = true;
            }
            if (progress.chapterTwentyTwoStep >= 12 && !progress.chapterTwentyTwoComplete)
            {
                progress.chapterTwentyTwoComplete = true;
                changed = true;
            }
            if (progress.chapterTwentyTwoComplete && progress.highestUnlockedChapter < 23)
            {
                progress.highestUnlockedChapter = 23;
                changed = true;
            }
            if (progress.chapterTwentyThreeStep >= 12 && !progress.chapterTwentyThreeComplete)
            {
                progress.chapterTwentyThreeComplete = true;
                changed = true;
            }
            if (progress.chapterTwentyThreeComplete && progress.highestUnlockedChapter < 24)
            {
                progress.highestUnlockedChapter = 24;
                changed = true;
            }
            if (progress.chapterTwentyFourStep >= 12 && !progress.chapterTwentyFourComplete)
            {
                progress.chapterTwentyFourComplete = true;
                changed = true;
            }
            if (progress.saveVersion < 24)
            {
                progress.saveVersion = 24;
                changed = true;
            }
            ...
            try
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    if (saved != null)
                    {
                        ...
                        if (saved.chapterEighteenComplete || saved.chapterEighteenStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 19);
                        }
                        if (saved.chapterNineteenComplete || saved.chapterNineteenStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 20);
                        }
                        if (saved.chapterTwentyComplete || saved.chapterTwentyStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 21);
                        }
                        if (saved.chapterTwentyOneComplete || saved.chapterTwentyOneStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 22);
                        }
                        if (saved.chapterTwentyTwoComplete || saved.chapterTwentyTwoStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 23);
                        }
                        if (saved.chapterTwentyThreeComplete || saved.chapterTwentyThreeStep >= 12)
                        {
                            saved.highestUnlockedChapter = Mathf.Max(saved.highestUnlockedChapter, 24);
                        }
